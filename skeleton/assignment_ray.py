...
# Custom tuple class with optional metadatas
class ATuple:
    """Custom tuple.
    ...
    def lineage(self,spanContext):
        # YOUR CODE HERE (ONLY FOR TASK 1 IN ASSIGNMENT 2)
        res = []
        l = self.operator.lineage([self],spanContext)
        for lst in l:
            for atuple in lst:
                res.append(atuple)
        return res
...
class Scan(Operator):
    ...
    Attributes:
        ...
        tracing (bool): Defines whether to start the tracing
        track_prov (bool): Defines whether to keep input-to-output
        mappings (True) or not (False).
        propagate_prov (bool): Defines whether to propagate provenance
        annotations (True) or not (False).
    ...
    def __init__(self, filepath, tracing = False, filter=None, track_prov=False,
                                              propagate_prov=False):
    ...
    # retrieve the file without using pandas
    def get_file(self):
        # read each line from the file and split the empty space 
        try:
            lst = []
    ...
    def get_next(self,spanContext):
        if not self.tracing: return self.get_next_deactivate(spanContext)
        if self.end_of_file: 
            self.curr = 0
            return None
        config = Config(
        ...
        logger.debug(tracer)
        with tracer.start_span('scan-op',child_of=spanContext) as scan_span:
            scan_span.set_tag('test-scan',str(self.curr))
            # initialize a batch
            batch = []
            # process each line from the file to a tuple, and add it to the block
            if self.cached == []:
                self.cached = self.get_file()
            ...
            self.curr += self.batch_size
            return batch
    # normal get next method if we decide to deactivate tracing 
    def get_next_deactivate(self,spanContext):
        if self.end_of_file: 
            self.curr = 0
            return None
        # initialize a batch
    ...
    def lineage(self, tuples,spanContext):
        # YOUR CODE HERE (ONLY FOR TASK 1 IN ASSIGNMENT 2)
        config = Config(
            config={
                'sampler': {
        ...
        tracer = config.new_tracer()
        logger.debug(tracer)
        with tracer.start_span('scan-op-lin',child_of=spanContext) as scan_span:
            self.batch = []
            for t in tuples:
...
class Join(Operator):
    ...
    def get_next(self,spanContext):
        ...
        tracer = config.new_tracer()
        logger.debug(tracer)
        with tracer.start_span('join-op',child_of=spanContext) as join_span:
            # declare a batch
            batch = []
            if not self.hasBuiltTable:
                # First, we load up all the tuples from the right input into the hash table
                while True:
                    right_upstream = ray.get(self.right_input.get_next.remote(join_span.context))
            ...
            left_upstream = ray.get(self.left_input.get_next.remote(join_span.context))
    ...
    # declare a batch
        ...
        if not self.hasBuiltTable:
            # First, we load up all the tuples from the right input into the hash table
            while True:
                left_upstream = ray.get(self.left_input.get_next.remote(spanContext))
                if left_upstream == None:
                    break
                
                for r in left_upstream:
                    if self.left_tuple_len == 0:
                        self.left_tuple_len = len(r.tuple)
                    key = r.tuple[self.left_join_attribute]
                    if self.track_prov:
                        if key not in self.left_table:
                            self.left_table[key] = [r]
                        else:
                            self.left_table[key].append(r)
                        #self.right_idx = self.right_idx + 1
                    if key in self.hashtable:
                        self.hashtable[r.tuple[self.left_join_attribute]].append(r)
                    else:
                        self.hashtable[key] = [r]
        self.hasBuiltTable = True
        # Then for each tuple in the left input, we match and yield the joined output tuple to batch
        right_upstream = ray.get(self.right_input.get_next.remote(spanContext))
        if right_upstream == None:
            return None
        for l in right_upstream:
            if self.right_tuple_len == 0:
                self.right_tuple_len = len(l.tuple)
            key = l.tuple[self.right_join_attribute]
            if self.track_prov:
                if key not in self.right_table:
                    self.right_table[key] = [l]
                else:
                    self.right_table[key].append(l)
            if key in self.hashtable:
                for r in self.hashtable[key]:
                    output = list(r.tuple)
                    output.extend(list(l.tuple))
                    output = tuple(output)
                    atuple = ATuple(tuple=output,operator=self)
                    if self.propagate_prov:
                        atuple.metadata = "("+r.metadata[1:-1] +"*"+ l.metadata[1:-1]+")"
                    batch.append(atuple)
    ...
    def lineage(self, tuples,spanContext):
        ...
        tracer = config.new_tracer()
        logger.debug(tracer)
        with tracer.start_span('join-op-lin',child_of=spanContext) as join_span:
            res = []
            for t in tuples:
                tmp = []
                lin = []
                ...
                if split_left[self.left_join_attribute] in self.left_table:
                    for i in self.left_table[split_left[self.left_join_attribute]]:
                        if split_left == i.tuple:
                            tmp+=ray.get(self.left_input.lineage.remote([i],join_span.context))
                if split_right[self.right_join_attribute] in self.right_table:
                    for i in self.right_table[split_right[self.right_join_attribute]]:
                        if split_right == i.tuple:
                            tmp+= ray.get(self.right_input.lineage.remote([i],join_span.context))
...
class Project(Operator):
    ...
    # Initializes project operator
    def __init__(self, input, fields_to_keep=[], tracing = False,track_prov=False,
                                                 propagate_prov=False):
        ...
        if self.propagate_prov and (self.input.handle == "Join" or self.input.handle == "Scan" or self.input.handle == "AVG"):
            self.prov_name = self.input.handle.upper()
    ...
    def get_next(self,spanContext):
        ...
        tracer = config.new_tracer()
        logger.debug(tracer)
        with tracer.start_span('project-op',child_of=spanContext) as project_span:
            lst = []
            # if upstream pass None, return None
            next_batch = ray.get(self.input.get_next.remote(project_span.context))
            if next_batch == None:
                return None
    ...
    # get_next method when tracing is deactivated
    def get_next_deactivate(self,spanContext):
        lst = []
        # if upstream pass None, return None
        next_batch = ray.get(self.input.get_next.remote(spanContext))
        if next_batch == None:
            return None
        # if fields to keep is empty, we assign to batch directly to list
        if self.fields_to_keep == []:
    ...
    def lineage(self, tuples,spanContext):
        ...
        tracer = config.new_tracer()
        with tracer.start_span('project-op-lin',child_of=spanContext) as project_span:
            res = []
            # YOUR CODE HERE (ONLY FOR TASK 1 IN ASSIGNMENT 2)
            for t in tuples:
                if (t.index,t.tuple) in self.cache_mapping:
                    input_mapping = self.cache_mapping[(t.index,t.tuple)]
                    res += ray.get(self.input.lineage.remote([input_mapping],project_span.context))
            return res
...
class GroupBy(Operator):
    ...
    def __init__(self, input, key, value, agg_fun, tracing = False,track_prov=False,
                                                   propagate_prov=False):
    ...
    def get_next(self,spanContext):
        ...
        config = Config(
            ...
            service_name='groupby',
            validate=True
        )
        tracer = config.new_tracer()
        logger.debug(tracer)
        with tracer.start_span('groupby-op',child_of=spanContext) as groupby_span:
            # declare a batch
            batch = []
            ...
            if self.key == None:            
                ...
                if not self.hasAllTuples:
                    ...
                    while True:
                        upstream = ray.get(self.input.get_next.remote(groupby_span.context))
                        if upstream == None: 
                            break
    ...
    # get next() method if tracing is deactivated
    def get_next_deactivate(self,spanContext):
        # declare a batch
        batch = []
        if self.end_of_batch: 
        ...
        if self.key == None:            
            ...
            if not self.hasAllTuples:
                # pulling from upstream
                while True:
                    upstream = ray.get(self.input.get_next.remote(spanContext))
                    if upstream == None: 
                        break
                    # then for each tuple, we update the tuple 
                    for atuple in upstream:
    ...
    def lineage(self, tuples,spanContext):
        ...
        tracer = config.new_tracer()
        with tracer.start_span('groupby-op-lin',child_of=spanContext) as groupby_span:
            res = []
            remove_dup = []
            # if we only doing aggregate, we simply pull the lineage from the previous operator
            if self.key == None:
                for atuple in self.cache:
                    lin= ray.get(self.input.lineage.remote([atuple],groupby_span.context))
                ...
                for t in tuples:
                    ...
                    for each in upstream_tuples:
                        lin = ray.get(self.input.lineage.remote([each],groupby_span.context))
    ...
    def how(self,tuples):
        res = ""
        for i in range(0, len(tuples)):
            res += self.handle +"("+tuples[i].metadata+"),"
        res = res[:-1]
        return res
...
class OrderBy(Operator):
    ...
    def __init__(self, input, comparator, ASC=True, tracing = False, track_prov=False,
                                                    propagate_prov=False):
    ...
    def get_next(self,spanContext):
        ...
        config = Config(
            ...
            service_name='orderby',
            validate=True
        )
        tracer = config.new_tracer()
        logger.debug(tracer)
        with tracer.start_span('orderby-op',child_of=spanContext) as orderby_span:
            # declare a batch
            batch = []
            # fetch all the batches upstream
            if not self.hasAllTuples:
                while True:
                    upstream = ray.get(self.input.get_next.remote(orderby_span.context))
                    #fetching done? then break out the loop
                    if upstream == None: break
                    self.res += upstream
    ...
    def lineage(self, tuples,spanContext):
        ...
        tracer = config.new_tracer()
        logger.debug(tracer)
        with tracer.start_span('orderby-op-lin',child_of=spanContext) as orderby_span:
            return ray.get(self.input.lineage.remote(tuples,orderby_span.context))
...
class TopK(Operator):
    ...
    def __init__(self, input, k=None, tracing = False, track_prov = False, propagate_prov=False):
    ...
    def get_next(self,spanContext):
        ...
        tracer = config.new_tracer()
        logger.debug(tracer)
        with tracer.start_span('topk',child_of=spanContext) as topk_span:
            # declare a batch
            batch = []
            # fetch all the batches upstream
            if not self.hasAllTuples:
                while True:
                    upstream = ray.get(self.input.get_next.remote(topk_span.context))
                    #fetching done? then break out the loop
                    if upstream == None: break
                    self.res += upstream
    ...
    # get_next() method if tracing is deactivated
    def get_next_deactivate(self,spanContext):
        # declare a batch
        batch = []
        #if we are done with all input
    ...
    def lineage(self, tuples,spanContext):
        ...
        tracer = config.new_tracer()
        logger.debug(tracer)
        with tracer.start_span('topk-op-lin',child_of=spanContext) as topk_span:
            return ray.get(self.input.lineage.remote(tuples,topk_span.context)) # we don't need to process intermediate data for this operator
