...
class Scan(Operator):
    ...
    def __init__(self, filepath, filter=None, track_prov=False,
        ...
        self.filter = filter
        self.reader=self.readf()
    #private method to generate tuple from reading file
    def readf(self):
...
class Join(Operator):
    ...
    #pull from upstream and process Join operation
    def __processJoin(self): 
    ...
    def get_next(self):
        ...
        # First, we load up all the tuples from the left input into the hash table
        try:
            return next(self.__processJoin())
        except StopIteration:
            return None
...
class Project(Operator):
    ...
    #we pull from upstream and process Project operation
    def __process(self):
...
class GroupBy(Operator):
    ...
    # pull from upstream, blocking, and process groupby operation
    def __processGroupby(self):
        if self.key == None:            
    ...
    def get_next(self):
        ...
        try:
            return next(self.__processGroupby())
        except StopIteration:
            return None
...
class Select(Operator):
    ...
    # we keep pulling the upstream's tuple and generate the filtered tuple
    def __pulling(self):
...
if __name__ == "__main__":
    ...
    def task1():
        ...
        for it in testGroupby.get_next():
            if it:
                aggr_val = repr(it)
