from assignment_12_generator import *
import pytest
import filecmp
import pdb
...
def testScan0():
    ...
    while True:
        ...
        if batch == None: break
        line = ' '.join(str(x) for x in batch.tuple)
        outF.write(line+"\n" )
    outF.close()
...
def testScan1():
    ...
    outF = open("testScan_test1.txt","w")
    while True:
        batch = testScan.get_next()
        if batch == None: break
        line = ' '.join(str(x) for x in batch.tuple)
        outF.write(line+"\n" )
    outF.close()
    assert len(filtered_lst) == len(total_batch), "incorrect scanned size with predicate function passed in,{}".format(len(total_batch))
